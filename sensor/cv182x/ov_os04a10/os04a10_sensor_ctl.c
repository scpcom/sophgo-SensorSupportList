...
static void os04a10_linear_640x480_90fps_12BIT_init(VI_PIPE ViPipe);
...
void os04a10_init(VI_PIPE ViPipe)
{
	...
	if (enWDRMode == WDR_MODE_2To1_LINE) {
		...
		else if (u8ImgMode == OS04A10_MODE_720P90_12BIT)
			os04a10_linear_640x480_90fps_12BIT_init(ViPipe);
	}
...
static void os04a10_linear_640x480_90fps_12BIT_init(VI_PIPE ViPipe)
{
	...
	os04a10_write_register(ViPipe, 0x3800, 0x02);		// crop x start
	os04a10_write_register(ViPipe, 0x3801, 0xbe);		// crop x start
	os04a10_write_register(ViPipe, 0x3804, 0x07);		// crop x end
	os04a10_write_register(ViPipe, 0x3805, 0xcd);		// crop x end

	os04a10_write_register(ViPipe, 0x3802, 0x01);		// crop y start
	os04a10_write_register(ViPipe, 0x3803, 0x8e);		// crop y start
	os04a10_write_register(ViPipe, 0x3806, 0x04);		// crop y end
	os04a10_write_register(ViPipe, 0x3807, 0x6d);		// crop y end

	// os04a10_write_register(ViPipe, 0x3810, 0x00);		// output x start
	// os04a10_write_register(ViPipe, 0x3811, 0x00);		// output x start
	// os04a10_write_register(ViPipe, 0x3812, 0x00);		// output y start
	// os04a10_write_register(ViPipe, 0x3813, 0x00);		// output y start
	os04a10_write_register(ViPipe, 0x3808, 0x05);		// output x size
	os04a10_write_register(ViPipe, 0x3809, 0x00);		// output x size
	...
	printf("ViPipe:%d,===OS04A10 720P 90fps 12bit LINE Init OK!===\n", ViPipe);
}
